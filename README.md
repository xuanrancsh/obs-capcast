...
- **投屏 / 音频映射各自独立开关**：每一项都有独立的「启用」勾选框 + 独立的「启动 / 停止」按钮，可单独开关，互不干扰
- **一键开始**：按勾选分别执行——勾了投屏就开投屏，勾了音频映射就开音频映射，都没勾就不做任何操作
- **一键停止**：停掉当前正在运行的全部（与勾选状态无关）
- **OBS 启动时自动开始**（总闸，默认关）：
  - 关闭 → OBS 启动后插件完全不动，相当于没启用
  - 开启 → 按**上次勾选**的项目自动启动（上次只勾了音频就只开音频，两项都勾就都开）
  - 上次两项都没勾 → 不做任何操作
- **全部参数自动记忆**：目标屏幕、音频设备、音量、音轨、投影源、各项勾选状态，重启 OBS 后保持
- **一键开始 / 一键停止**（工具菜单 + 全局热键）
- 自动**全屏投影**到采集卡副屏（节目 PGM / 预览 PVW 可选）
- 停止时**只关闭本插件自己打开的投影窗口**，不会误关你在 OBS 里手动打开的预览 / 多视图投影
...
3. 想长期怎么用，就勾好「启用投屏 / 启用音频映射」；想临时单独开关，直接用该项右侧的按钮
4. 若希望**每次打开 OBS 就自动开始**，勾选「OBS 启动时自动开始」——它会照着第 3 步的勾选自动执行
5. 点 **一键开始**（或等 OBS 自动启动）：投屏打开、音频进采集卡
6. 第二台电脑在采集卡输入端看到画面 + 声音，无需任何额外设置
...
**Q：我的副屏是 4K，为什么面板里显示 1280x720？是不是按 720p 投出去的？**

不是。面板里的分辨率是显示器的**物理分辨率**（4K 屏会显示 3840x2160）；旁边的
「系统缩放」提示说明的是 Windows 的显示缩放把 OBS 看到的"逻辑尺寸"除小了
（4K 在 300% 缩放下逻辑值正好是 1280x720）。这只是读数的换算差异，
**不影响投屏画质** —— 投影窗口是全屏投到该物理显示器上的。

真正决定画面清晰度的是 **OBS 的画布 / 输出分辨率**（设置 → 视频）。若 OBS 画布是
1920x1080，即使副屏是 4K，投出去的画面内容也只有 1080p。想要 4K 画面，
需要把 OBS 的画布和输出分辨率一并设为 3840x2160。
